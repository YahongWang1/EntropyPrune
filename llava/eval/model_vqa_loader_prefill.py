...
def configure_EntropyPrune(model, args):
    """Configure EntropyPrune for the current evaluation run."""
    ...
    if args.sparse:
        ...
        model.config.EntropyPrune_config = EntropyPrune_config
...
def eval_model(args):
    ...
    configure_EntropyPrune(model, args)  # Configure EntropyPrune before inference.
    ...
    prefill_time = (end_time - start_time) * 1000  # 转换为毫秒 (ms)
    # print(f"Prefilling Time: {prefill_time:.2f} ms")
    print(f"Latency Time: {prefill_time:.2f} ms")
